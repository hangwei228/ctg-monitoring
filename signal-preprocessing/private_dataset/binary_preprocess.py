# -*- coding: utf-8 -*-
"""
数据预处理方法 - 二分类版本 + Q值筛选 + 去噪
(在 三分类 基础上将 "无反应(异常)" 与 "可疑" 合并为一类)
...
class Config:
    ...
    # 输出路径 (二分类单独子目录, 避免覆盖三分类结果)
    OUTPUT_PATH = r'./data/output/binary_result'

    # 标签映射 (二分类: 无反应 + 可疑 合并为异常类)
    LABEL_MAP = {
        '有反应': 0,
        '无反应': 1,
        '可疑': 1
