...
class Config:
    # 原始数据路径
    RAW_DATA_PATH = os.path.join(BASE_DIR, '原始数据集')

    # 输出路径
    OUTPUT_PATH = os.path.join(BASE_DIR, '二分类', 'output')

    # 标签映射 (二分类: 有反应=0, 异常(无反应+可疑)=1)
    LABEL_MAP = {
        '有反应': 0,
        '无反应': 1,
        '可疑': 1
...
# 基础去噪方法
# ==============================================================================
def basic_denoise(fhr_signal, uc_signal):
    """基础去噪：FHR裁剪到40-220，NaN置为140；UC将NaN置为0"""
...
def parse_csv_labels(csv_path):
    ...
    for encoding in encodings:
        try:
            ...
            # 检测第一行是否为表头（包含"编号"和"评价"）
            first_line = lines[0].strip()
            first_headers = [h.strip() for h in first_line.split(',')]

            if '编号' in first_headers and '评价' in first_headers:
                # 第一行就是表头（如 ActiveLearningCTG.csv）
...
def main():
    ...
    for i, pair in enumerate(data_pairs):
        ...
        # 基础去噪
        fhr_denoised, uc_denoised = basic_denoise(fhr_raw, uc_raw)
