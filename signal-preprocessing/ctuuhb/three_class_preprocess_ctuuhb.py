# -*- coding: utf-8 -*-
"""
CTU-UHB 数据集预处理 - 三分类版本 (基于 pH 分档)
方法: Q值筛选 + 泓至去噪 + FHR异常值处理 + 基线提取 + 标准化 + 数据增强 + 长度统一
...
class Config:
    ...
    # 分类模式
    CLASS_NAMES = {0: '正常', 1: '临界', 2: '酸血症'}
    NUM_CLASSES = 3

    # pH 阈值 (三分类分档)
    PH_HIGH = 7.2       # pH > 7.2 -> 0
    PH_LOW = 7.15       # pH < 7.15 -> 2; 中间 -> 1
...
def ph_to_label(ph, config):
    """三分类(按pH分档):
       pH > 7.2 -> 0(正常); 7.15<=pH<=7.2 -> 1(临界); pH<7.15 -> 2(酸血症)"""
    if ph > config.PH_HIGH:
        return 0
    elif ph >= config.PH_LOW:
        return 1
    else:
        return 2
...
def main():
    ...
    print(f'输出路径: {config.OUTPUT_PATH}')
    print(f'pH分档: >{config.PH_HIGH}->正常(0); {config.PH_LOW}-{config.PH_HIGH}->临界(1); <{config.PH_LOW}->酸血症(2)')
