...
产前 CTG 多模态 CNN 分类实现。三路 1D 残差卷积编码器（胎心率FHR / 宫缩UC / 胎动FM）+ SE 注意力，拼接临床
stats 向量（孕周、年龄、CTG 指南特征）后接全连接分类头；
...
python run.py --dataset all --task both          # center, 二分类+三分类
python run.py --dataset center --task binary            # 二分类
python run.py --dataset center --task three_class       # 三分类
python run.py --dataset center --task binary --quick_test
```

| 参数             | 可选值                              | 默认          | 说明       |
| ---------------- | ----------------------------------- | ------------- | ---------- |
| `--dataset`    | `center`                          | `center`    | 数据集     |
...
| ------------------------- | ---------- | ---------- | ---------- |
| `binary（二分类）`      | `0.8394` | `0.9234` | `0.8392` |
| `three_class（三分类）` | `0.8404` | `0.9250` | `0.8362` |
